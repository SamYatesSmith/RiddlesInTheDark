...
class RiddleGame:
    ...
    def load_questions(self, filepath):
        ...
        try:
            ...
            print(
                wrap_text(
                    "Error: The file path provided does not exist.  Please "
                    "check the file path and give it another whirl."
                )
            )
    ...
    def choose_game_mode(self):
        ...
        while True:
            try:
                mode = input(
                    wrap_text(
                        "Choose your game mode: Key '1' for Single player and"
                        "'2' for 2 player. 1/2:\n"
                    )
                )
                ...
                print(
                    wrap_text(
                        "Invalid input. Please enter '1' for Single player"
                        "game, or '2' for Two player game."
                    )
                )
    ...
    def get_user_input(self, prompt, valid_responses=None, lower=True):
        ...
        while True:
            ...
            if valid_responses is None or user_input in valid_responses:
                return user_input
            print(f"Invalid input.  Please enter one of the"
                  "{valid_responses}.")
    ...
    intro_message = (
        ...
        "Riddles in the Dark is a short riddle based game. In order to claim "
        "victory, the player must accrue as few points as possible. Points "
        "are incremented by answering riddles incorrectly, asking for hints, "
        "using too many hints and skipping the current question.\n\n"
        "Players are allowed 5 hints per question. These hints will accrue "
        "1 point each.\n\n"
        "However, if a player uses more than 5 hints, they are punished with "
        "an incorrect answer's point value.\n\n"
        "When a player answers incorrectly, they receive 2 points.\n\n"
        "When a player skips the question, they will receive a 10 point "
        "penalty.\n\n"
        "Players are provided with underscores which represent the number of "
        "letters in the answer.\n\n"
        "Good luck, and Have fun!\n\n"
    )
    paragraphs = intro_message.split("\n\n")
    wrapped_intro = "\n\n".join(
        textwrap.fill(paragraph, width=79) for paragraph in paragraphs
    )
    ...
    def display_letter_count_hint(self, answer):
        """
        Dispalys a hint for the current riddle, shows the number of letters in
        the answer as underscores
        """
    ...
    def handle_question(self, question, answer, hints):
        """
        Handles logic for asking a question. Takes users response, provides
        hint and skips
        """
        question_text = (
            f"Question for {self.player_names[self.current_player]}: "
            f"{question}"
        )
        ...
        while True:
            ...
            if response.lower() == "skip":
                ...
                print(
                    wrap_text(
                        f"Skipped.  This costs you 10 points.  Current point "
                        f"total: {self.player_scores[self.current_player]}"
                    )
                )
                break
    ...
    def play_game(self):
        """
        Starts and manages games flow, from choosing the games mode to
        conclusion
        """
        ...
        ready_prompt = (
            "Ok, so are you ready to play? (yes/no): "
            if self.player_mode == 1
            else "Ok, so are you both ready to play? (yes/no): "
        )
        ...
        if self.player_mode == 1:
            ...
            for i in range(questions_per_player * 2):
                current_question_index = (
                    i // 2 if i % 2 == 0 else (i // 2) + questions_per_player
                )
                print(
                    f"{self.player_names[self.current_player]}'s turn "
                    f"(Question "
                    f"{current_question_index % questions_per_player + 1} "
                    f"of {questions_per_player}): "
                )
                self.handle_turn(current_question_index)
                self.next_player()

        if self.player_mode == 2 and self.player_scores[0] == \
    ...
    def conclude_game(self):
        ...
        if self.player_mode == 1:
            ...
            print(
                wrap_text(
                    f"{self.player_names[0]}'s total points: "
                    f"{self.player_scores[0]}, {self.player_names[1]}'s total "
                    f"points: {self.player_scores[1]}."
                )
            )
            if self.player_scores[0] < self.player_scores[1]:
                if self.player_scores[0] == 0:
                    print(
                        wrap_text(
                            f"Flawless Victory! Incredible effort, "
                            f"{self.player_names[0]}. You're a riddle hero! "
                            f"Gollum would stand no chance!"
                        )
                    )
                else:
    ...
    def sudden_death(self):
        ...
        print(
            wrap_text(
                f"It's a tie! Both players have {self.player_scores[0]} points"
            )
        )
        print("Entering the Sudden Death round...")
        print(
            wrap_text(
                "Hints are disabled in Sudden Death.  A wrong answer means "
                "you loose.  Good luck!"
            )
        )
        ...
        while sudden_death_index < len(self.questions):
            for player_index in range(2):
                ...
                question, answer = (
                    self.questions[sudden_death_index],
                    self.answers[sudden_death_index],
                )
                ...
                if response.lower() != answer.lower():
                    print(
                        wrap_text(
                            f"It's a tie! Both players have "
                            f"{self.player_scores[0]} points."
                        )
                    )
                    winner = self.player_names[1 - self.current_player]
                    print(
                        wrap_text(
                            f"Congratulations! {winner}, you won the game! "
                            f"{winner} is the Riddle King."
                        )
                    )
                    return
                ...
                if sudden_death_index >= len(self.questions):
                    print(
                        wrap_text(
                            "Wow wow wow wow.  Theres some very accomplished "
                            "riddling taking place here.  I hold my hands up "
                            "and gracefully admit defeat.  Congratulations, "
                            "you both beat me, so you both win!"
                        )
                    )
                    return
